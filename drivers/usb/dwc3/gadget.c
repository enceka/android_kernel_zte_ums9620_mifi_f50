...
static int __dwc3_gadget_ep_enable(struct dwc3_ep *dep, unsigned int action)
{
	...
	if (dep->endpoint.android_kabi_reserved1 == 0xff) {
		...
		if (DWC3_DEPCMD_STATUS(reg)) {
			dev_err(dwc->dev, "%s %s depcmd(0x%x) error\n",
				__func__, dep->name, reg);
			queue_work(system_unbound_wq, &dwc->reconnect_work);
		}
	} else {
#endif
...
static void dwc3_gadget_reconnect_work(struct work_struct *work)
{
...
int dwc3_gadget_init(struct dwc3 *dwc)
{
	...
	INIT_WORK(&dwc->reconnect_work, dwc3_gadget_reconnect_work);
