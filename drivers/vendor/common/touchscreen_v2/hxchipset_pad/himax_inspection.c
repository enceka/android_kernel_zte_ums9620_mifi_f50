...
#else
	if (file_w_flag) {
		tpd_copy_to_tp_firmware_data(g_rslt_data);
	}
#endif
...
UPDATE_MPFW_FAIL:
	...
	for (j = 0; j < HX_CRITERIA_ITEM - 1; j++) {
		...
		if (g_test_item_flag[j] == 1) {
			...
			if (j <= i) {
				I("%s %s : %s\n", __func__, g_hx_inspt_mode[j],
					(rslt & (1 << j)) ? "Fail":"OK");
				if ((rslt & (1 << j))) {
					if (strnstr(g_hx_inspt_mode[j], "OPEN",
						strlen(g_hx_inspt_mode[j]))) {
						himax_tptest_result = himax_tptest_result | TP_OPEN_TEST_FAIL;
					} else if (strnstr(g_hx_inspt_mode[j], "SHORT",
						strlen(g_hx_inspt_mode[j]))) {
						himax_tptest_result = himax_tptest_result | TP_SHORT_TEST_FAIL;
					} else if (strnstr(g_hx_inspt_mode[j], "NOISE",
						strlen(g_hx_inspt_mode[j]))) {
						himax_tptest_result = himax_tptest_result | TP_NOISE_TEST_FAIL;
					} else {
						himax_tptest_result = himax_tptest_result | TP_RAWDATA_TEST_FAIL;
					}
				}
			}
		}
