...
unlock:
    ...
    cts_wait_current_mode(cts_dev, CTS_KRANG_NORMAL_MODE);
...
int cts_test_short(struct cts_device *cts_dev, struct cts_test_param *param)
{
    ...
    if (driver_validate_data) {
        test_ret = validate_tsdata(cts_dev, "GND-short", test_result, param->invalid_nodes,
            param->num_invalid_node, validate_data_per_node, param->min, param->max);
    ...
    for (loopcnt = 0; loopcnt < SHORT_COLS_TEST_LOOP; loopcnt++) {
        ...
        if (driver_validate_data) {
            test_ret = validate_tsdata(cts_dev, "Col-short", test_result, param->invalid_nodes,
                param->num_invalid_node, validate_data_per_node, param->min, param->max);
    ...
    for (loopcnt = 0; loopcnt < SHORT_ROWS_TEST_LOOP; loopcnt++) {
        ...
        if (driver_validate_data) {
            test_ret = validate_tsdata(cts_dev, "Row-short", test_result, param->invalid_nodes,
                param->num_invalid_node, validate_data_per_node, param->min, param->max);
            if (selftestdata->shortdata != NULL) {
                memset(selftestdata->shortdata, 0, RAWDATA_BUFFER_SIZE(cts_dev));
                memcpy(selftestdata->shortdata, test_result, RAWDATA_BUFFER_SIZE(cts_dev));
