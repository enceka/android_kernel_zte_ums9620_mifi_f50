...
int axs_read_file(char *file_name, u8 **file_buf)
{
    ...
    {
        AXS_INFO("filename/filebuf is NULL");
        return -EINVAL;
    }
    ...
    if (IS_ERR(filp))
    {
        AXS_INFO("open %s file fail", file_path);
        return -ENOENT;
    }
    ...
    *file_buf = (u8 *)vmalloc(file_len);
    ...
    if (ret < 0)
        AXS_INFO("read file fail");
    AXS_INFO("file len:%x read len:%x pos:%x", (u32)file_len, ret, (u32)pos);
    filp_close(filp, NULL);
    set_fs(old_fs);
    ...
    ret = request_firmware(&fw, file_name, g_axs_data->dev);
    if (ret == 0) {
        AXS_INFO("firmware(%s) request successfully", file_name);
    } else {
        AXS_ERROR("%s:firmware(%s) request fail,ret=%d\n",  __func__, file_name, ret);
#ifdef AXS_DEFAULT_FIRMWARE
        AXS_INFO("try to request default fw %s\n", axs_default_firmware_name);
        ret = request_firmware(&fw, axs_default_firmware_name, g_axs_data->dev);
        if (ret < 0) {
            AXS_ERROR("request default fw fail - %d\n", ret);
            goto out;
        } else {
            AXS_INFO("request default fw success\n");
        }
#endif
    }
...
static ssize_t axs_tprwreg_store(struct device *dev, struct device_attribute *attr, const char *buf, size_t count)
{
    ...
    if (1 == op_type) // read
    {
        ...
        AXS_INFO("wt_len:%d,rd_len:%d",wt_len,rd_len);
        ...
        ret = axs_write_bytes_read_bytes(g_axs_data->debug_tx_buf,wt_len,g_axs_data->debug_rx_buf,rd_len);
        AXS_INFO("axs_write_bytes_read_bytes ret=%d, cmd:\n",ret);
        for (i=0; i<wt_len; i++)
        {
            AXS_INFO("0x%02x,", g_axs_data->debug_tx_buf[i]);
        }

        AXS_INFO("read result:\n");
        for (i=0; i<rd_len; i++)
        {
            AXS_INFO("0x%02x,", g_axs_data->debug_rx_buf[i]);
        }
        s_rd_len = rd_len;
    }
    else if (0 == op_type)  // write
    {
        ...
        ret = axs_write_bytes(g_axs_data->debug_tx_buf, wt_len);
        if (ret < 0)
        {
            AXS_ERROR("axs_write_bytes fail, cmd:\n");
            for (i=0; i<wt_len; i++)
            {
                AXS_INFO("0x%02x,", g_axs_data->debug_tx_buf[i]);
            }
...
static ssize_t axs_fwupgrade_store(struct device *dev, struct device_attribute *attr, const char *buf, size_t count)
{
    ...
    {
        AXS_INFO("fw bin name's length(%d) fail", (int)count);
        return -EINVAL;
    }
    ...
    AXS_INFO("upgrade firmware by sysfs");
    mutex_lock(&s_device_mutex);
...
static ssize_t AXS_INFO_str_show(struct device *dev, struct device_attribute *attr, char *buf)
{
    int count;
    int ret;
    u8 write_reg[5] = {0x13,0x31,0x41,0x51,AXS_FREG_DEBUG_LEN_READ}; // read tx,rx
    u8 debug_str_len[2]= {0,0};
    ret = axs_write_bytes_read_bytes(write_reg,5,debug_str_len,2);
    if (ret < 0)
    {
    ...
    ret = axs_write_bytes_read_bytes(write_reg,5,g_axs_data->debug_rx_buf,(debug_str_len[0]<<8)|debug_str_len[1]);
    if (ret < 0)
    {
...
static ssize_t AXS_INFO_str_store(struct device *dev, struct device_attribute *attr, const char *buf, size_t count)
{
    return -EPERM;
}
#if AXS_DEBUG_LOG_EN
...
static ssize_t axs_log_en_store(struct device *dev, struct device_attribute *attr, const char *buf, size_t count)
{
    ...
    AXS_INFO("set log enable:%d",g_log_en);
    mutex_unlock(&s_device_mutex);
    return count;
}
#endif
static ssize_t axs_diff_show(struct device *dev, struct device_attribute *attr, char *buf)
{
    ...
    u8 cols = 0;
    int index = 0;
    s16 *diff_data_words = NULL;

    ret = axs_write_bytes_read_bytes(write_reg,5,tx_rx_len,2);
    if (ret < 0)
    {
    ...
    ret = axs_read_bytes(g_axs_data->debug_rx_buf, tx_rx_len[0] * tx_rx_len[1] * 2);
    if (ret < 0)
    {
        AXS_ERROR("read diffdat fail");
        return -EPERM;
    }
    diff_data_words = kzalloc(rows * cols * 2, GFP_KERNEL);
    if (diff_data_words == NULL) {
        AXS_ERROR("Alloc diffdata buffer failed");
        return -ENOMEM;
    }
    mutex_lock(&s_device_mutex);
    for (i = 0, index = 0; index < rows * cols; i += 2, index++) {
        diff_data_words[index] = (g_axs_data->debug_rx_buf[i] << 8) + g_axs_data->debug_rx_buf[i + 1];
    }
    for (i = 0; i < rows; i++) {
        pr_cont("AXS_CTP[%2d]", (i + 1));
        count += snprintf(buf + count, (PAGE_SIZE - count), "AXS_CTP[%2d]", (i + 1));
        for (j = 0; j < cols; j++) {
            pr_cont("%5d,",  diff_data_words[i * cols + j]);
            count += snprintf(buf + count, (PAGE_SIZE - count), "%5d,", diff_data_words[i * cols + j]);
        }
        pr_cont("\n");
        count += snprintf(buf + count, (PAGE_SIZE - count), "\n");
    }
    kfree(diff_data_words);
    mutex_unlock(&s_device_mutex);
    write_reg[4]=0x01; // read rawdata
    ret = axs_write_bytes(write_reg,5);
    if (ret < 0)
    {
        AXS_ERROR("write diffdata fail");
        return -EPERM;
    }
...
static ssize_t axs_rawdata_show(struct device *dev, struct device_attribute *attr, char *buf)
{
    ...
    raw_data_words = kzalloc(rows * cols * 2, GFP_KERNEL);
    if (raw_data_words == NULL) {
        AXS_ERROR("Alloc rawdata buffer failed");
        return -ENOMEM;
    }
    mutex_lock(&s_device_mutex);
    for (i = 0, index = 0; index < rows * cols; i += 2, index++) {
        raw_data_words[index] = (g_axs_data->debug_rx_buf[i] << 8) + g_axs_data->debug_rx_buf[i + 1];
    }
    ...
    for (i = 0; i < rows; i++) {
        ...
        count += snprintf(buf + count, (PAGE_SIZE - count), "AXS_CTP[%2d]", (i + 1));
        for (j = 0; j < cols; j++) {
            pr_cont("%5d,",  raw_data_words[i * cols + j]);
            count += snprintf(buf + count, (PAGE_SIZE - count), "%5d,", raw_data_words[i * cols + j]);
        }
        pr_cont("\n");
        count += snprintf(buf + count, (PAGE_SIZE - count), "\n");
    }
    kfree(raw_data_words);
    mutex_unlock(&s_device_mutex);
...
static ssize_t axs_gesture_store(struct device *dev, struct device_attribute *attr, const char *buf, size_t count)
{
    ...
    AXS_INFO("set gesture enable:%2d",g_axs_data->gesture_enable);
    mutex_unlock(&s_device_mutex);
    return count;
}
...
static ssize_t axs_gesture_show(struct device *dev, struct device_attribute *attr, char *buf)
{
    ...
    count = snprintf(buf, PAGE_SIZE, "get gesture enable:%2d\n", g_axs_data->gesture_enable);
    mutex_unlock(&s_device_mutex);
    return count;
}
#endif
...
static ssize_t axs_psensor_store(struct device *dev, struct device_attribute *attr, const char *buf, size_t count)
{
    //struct input_dev *input_dev = g_axs_data->input_dev;
    mutex_lock(&s_device_mutex);
    g_axs_data->psensor_enable = hex_to_u8(&buf[0],2);
    axs_write_psensor_enable(g_axs_data->psensor_enable);
    AXS_INFO("set psensor enable:%2d",g_axs_data->psensor_enable);
    mutex_unlock(&s_device_mutex);
    return count;
}

static ssize_t axs_psensor_show(struct device *dev, struct device_attribute *attr, char *buf)
{
    //struct input_dev *input_dev = g_axs_data->input_dev;
    int count;
    mutex_lock(&s_device_mutex);
    axs_read_psensor_enable(&g_axs_data->psensor_enable);
    count = snprintf(buf, PAGE_SIZE, "get psensor enable:%2d\n", g_axs_data->psensor_enable);
    mutex_unlock(&s_device_mutex);
    return count;
}
#endif
...
static ssize_t axs_esd_store(struct device *dev, struct device_attribute *attr, const char *buf, size_t count)
{
    //struct input_dev *input_dev = g_axs_data->input_dev;
    mutex_lock(&s_device_mutex);
    g_axs_data->esd_host_enable = hex_to_u8(&buf[0],2);
    AXS_INFO("set esd check enable:%d",g_axs_data->esd_host_enable);
    mutex_unlock(&s_device_mutex);
    return count;
}
...
static ssize_t axs_esd_show(struct device *dev, struct device_attribute *attr, char *buf)
{
    ...
    int count;
    mutex_lock(&s_device_mutex);
    count = snprintf(buf, PAGE_SIZE, "get esd check enable: %2d\n", g_axs_data->esd_host_enable);
    mutex_unlock(&s_device_mutex);
    return count;
}
#endif
...
#if AXS_HEADSET_EN
static ssize_t axs_headset_en_store(struct device *dev, struct device_attribute *attr, const char *buf, size_t count)
{
    u8 cmd_type[2] = {AXS_REG_HEADSET_WRITE, 1};

    mutex_lock(&s_device_mutex);
    g_axs_data->headset_enable = hex_to_u8(&buf[0],2);
	cmd_type[1] = g_axs_data->headset_enable;

    axs_write_buf(cmd_type, 2);

    AXS_INFO("set headset enable:%2d",g_axs_data->headset_enable);
    mutex_unlock(&s_device_mutex);
    return count;
}

static ssize_t axs_headset_en_show(struct device *dev, struct device_attribute *attr, char *buf)
{
    int count;
    mutex_lock(&s_device_mutex);
    count = snprintf(buf, PAGE_SIZE, "get headset enable:%2d\n", g_axs_data->headset_enable);
    mutex_unlock(&s_device_mutex);
    return count;
}
#endif
...
#if AXS_CHARGE_EN
static ssize_t axs_charge_en_store(struct device *dev, struct device_attribute *attr, const char *buf, size_t count)
{
    u8 cmd_type[2] = {AXS_REG_CHARGE_WRITE, 1};

    mutex_lock(&s_device_mutex);
    g_axs_data->charge_enable = hex_to_u8(&buf[0],2);
	cmd_type[1] = g_axs_data->charge_enable;

    axs_write_buf(cmd_type, 2);

    AXS_INFO("set charge enable:%2d",g_axs_data->charge_enable);
    mutex_unlock(&s_device_mutex);
    return count;
}

static ssize_t axs_charge_en_show(struct device *dev, struct device_attribute *attr, char *buf)
{
    int count;
    mutex_lock(&s_device_mutex);
    count = snprintf(buf, PAGE_SIZE, "get charge enable:%2d\n", g_axs_data->charge_enable);
    mutex_unlock(&s_device_mutex);
    return count;
}
#endif
...
#if AXS_ROTATION_EN
static ssize_t axs_rotation_mode_store(struct device *dev, struct device_attribute *attr, const char *buf, size_t count)
{
    u8 cmd_type[2] = {AXS_REG_ROTATION_WRITE, 0};

    mutex_lock(&s_device_mutex);
    g_axs_data->rotation_mode = hex_to_u8(&buf[0],2);
	cmd_type[1] = g_axs_data->rotation_mode;

    axs_write_buf(cmd_type, 2);

    AXS_INFO("set rotation mode:%2d",g_axs_data->rotation_mode);
    mutex_unlock(&s_device_mutex);
    return count;
}

static ssize_t axs_rotation_mode_show(struct device *dev, struct device_attribute *attr, char *buf)
{
    int count;
    mutex_lock(&s_device_mutex);
    count = snprintf(buf, PAGE_SIZE, "get rotation mode:%2d\n", g_axs_data->rotation_mode);
    mutex_unlock(&s_device_mutex);
    return count;
}
#endif
...
#if AXS_PROXIMITY_SENSOR_EN
static DEVICE_ATTR (axs_psensor_enable, S_IRUGO | S_IWUSR, axs_psensor_show, axs_psensor_store);
#endif
...
static DEVICE_ATTR(axs_driver_debug_str, S_IRUGO | S_IWUSR, AXS_INFO_str_show, AXS_INFO_str_store);
#if AXS_DEBUG_LOG_EN
static DEVICE_ATTR(axs_log_en, S_IRUGO | S_IWUSR, axs_log_en_show, axs_log_en_store);
#endif

#if AXS_HEADSET_EN
static DEVICE_ATTR(axs_headset_en, S_IRUGO | S_IWUSR, axs_headset_en_show, axs_headset_en_store);
#endif

#if AXS_CHARGE_EN
static DEVICE_ATTR(axs_charge_en, S_IRUGO | S_IWUSR, axs_charge_en_show, axs_charge_en_store);
#endif

#if AXS_ROTATION_EN
static DEVICE_ATTR(axs_rotation_mode, S_IRUGO | S_IWUSR, axs_rotation_mode_show, axs_rotation_mode_store);
#endif
...
#endif
#if AXS_PROXIMITY_SENSOR_EN
    &dev_attr_axs_psensor_enable.attr,
#endif
#if AXS_ESD_CHECK_EN
...
#endif
#if AXS_HEADSET_EN
	&dev_attr_axs_headset_en.attr,
#endif
#if AXS_HEADSET_EN
	&dev_attr_axs_charge_en.attr,
#endif
#if AXS_ROTATION_EN
	&dev_attr_axs_rotation_mode.attr,
#endif
    NULL
};
...
bool axs_debug_create_sysfs(struct axs_ts_data *ts_data)
{
    ...
    if (ret)
    {
        AXS_INFO("[EX]: sysfs_create_group() failed!!");
        sysfs_remove_group(&(client->dev.kobj), &axs_attribute_group);
        return false;
    }
    else
    {
        AXS_INFO("[EX]: sysfs_create_group() succeeded!!");
    }
...
static ssize_t axs_proc_write(struct file *filp, const char __user *buff, size_t count, loff_t *ppos)
{
    ...
    switch (ts_data->proc_opmode)
    {
        ...
        case PROC_WRITE_CMD:
            ...
            ret = axs_write_bytes(writebuf + 1, writelen);
            if (ret < 0)
            {
                AXS_ERROR("axs_write_bytes error");
            }
            break;
        default:
            break;
...
static ssize_t axs_proc_read(struct file *filp, char __user *buff, size_t count, loff_t *ppos)
{
    ...
    {
        case PROC_READ_CMD:
            ret = axs_read_bytes(g_axs_data->debug_rx_buf, count);
            if (ret < 0)
            {
...
int axs_create_proc_file(struct axs_ts_data *ts_data)
{
    ...
    {
        AXS_INFO("axs_create_proc_file success!");
    }
    return 0;
}
