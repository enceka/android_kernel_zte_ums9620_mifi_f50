...
#if AXS_BUS_SPI
int axs_write_bytes_read_bytes_onecs(u8 *cmd, u16 cmdlen, u8 *data, u16 datalen) // axs_write_read_onecs
{
    int ret = 0;
    ...
    {
        AXS_INFO("axs_write_bytes_read_bytes_onecs, fail:%d\n",ret);
...
int axs_read_bytes(u8 *rdbuf, u16 rdlen) // axs_read
{
    int ret = 0;
    struct spi_message msg;
    struct spi_transfer transfer;
    ...
    {
        AXS_ERROR("axs_read_bytes g_axs_data/device/data/datalen(%d) is invalid", rdlen);
        return -EINVAL;
    }
    spi_message_init(&msg);
    memset(&transfer, 0, sizeof(transfer));
    ...
    if (ret!=0)
    {
        AXS_INFO("axs_read_bytes, status= %d\n",ret);
        tpd_zlog_record_notify(TP_SPI_R_ERROR_NO);
    }
    mutex_unlock(&ts_data->bus_mutex);
    return ret;
...
int axs_write_bytes(u8 *writebuf, u16 writelen) // axs_write
{
    int ret = 0;
    struct spi_message msg;
    struct spi_transfer transfer;
    ...
    {
        AXS_ERROR("axs_write_bytes g_axs_data/device/data/datalen(%d) is invalid", writelen);
        return -EINVAL;
    }
    spi_message_init(&msg);
    memset(&transfer, 0, sizeof(transfer));
    ...
    if (ret!=0)
    {
        AXS_INFO("axs_write_bytes, status= %d\n",ret);
...
int axs_write_bytes_read_bytes(u8 *cmd, u16 cmdlen, u8 *data, u16 datalen) // axs_write_read
{
    int ret = 0;
    ret = axs_write_bytes(cmd, cmdlen);
    if (ret)
    {
        return ret;
    }
    ret = axs_read_bytes(data, datalen);
    return ret;
}
