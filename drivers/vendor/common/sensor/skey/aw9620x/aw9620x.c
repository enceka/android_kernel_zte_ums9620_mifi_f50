...
static uint8_t is_skey_exist = 0;
#ifdef CONFIG_VENDOR_ZTE_MISC_COMMON
#include <vendor/common/zte_misc.h>

int is_skey_exist_get(char *val, const void *arg)
{
	return snprintf(val, PAGE_SIZE, "%d", is_skey_exist);
}

static struct zte_misc_ops is_skey_exist_node = {
	.node_name = "is_skey_exist",
	.set = NULL,
	.get = is_skey_exist_get,
	.free = NULL,
	.arg = NULL,
};
#endif
	...
	switch (reg_val) {
		...
		case AW96205DNR_CHIP_ID:
			...
			if (aw9620x->sar_num == 0)
				is_skey_exist = 1;  /* add boardtest interface */
		break;
...
static int32_t aw9620x_soft_reset(struct aw9620x *aw9620x)
{
	...
	if (ret < 0) {
		AWLOGE(aw9620x->dev, "set REG_APB_ACCESS_EN err: %d", ret);
		return -AW_ERR;
	}

	ret = aw9620x_i2c_write(aw9620x, AW_REG_FLASH_WAKE_UP, AW_REG_FLASH_WAKE_UP_ON);
	if (ret < 0) {
		AWLOGE(aw9620x->dev, "set AW_REG_FLASH_WAKE_UP err: %d", ret);
		return -AW_ERR;
	}

	msleep(1);
	ret = aw9620x_i2c_write(aw9620x, REG_MCFG00, REG_MCFG00_EN);
	if (ret < 0) {
		AWLOGE(aw9620x->dev, "set REG_MCFG00 err: %d", ret);
		return -AW_ERR;
	}

	ret = aw9620x_i2c_write(aw9620x, REG_RSTNALL, REG_RSTNALL_VAL);
	if (ret < 0) {
		AWLOGE(aw9620x->dev, "set REG_RSTNALL err: %d", ret);
		return -AW_ERR;
	}

	msleep(AW_POWER_ON_DELAY_MS);

	ret = aw9620x_i2c_write(aw9620x, AW_REG_FLASH_WAKE_UP, AW_REG_FLASH_WAKE_UP_OFF);
	if (ret != AW_OK) {
		...
		ret = aw9620x_i2c_write(aw9620x, AW_REG_FLASH_WAKE_UP, AW_REG_FLASH_WAKE_UP_OFF);
		if (ret < 0) {
			AWLOGE(aw9620x->dev, "set AW_REG_FLASH_WAKE_UP err1: %d", ret);
			return -AW_ERR;
		} else {
...
void aw9620x_irq_handle_func(struct aw9620x *aw9620x)
{
	...
	AWLOGI(aw9620x->dev, "IRQSRC = 0x%x STAT0 = 0x%x", irq_status, curr_status_val);
	...
	for (i = 0; i < AW_CHANNEL_NUM_MAX; i++) {
		...
		case TRIGGER_FAR:
			...
			if (aw9620x->sar_num == 0) {
				...
				AWLOGI(aw9620x->dev, "skey0 report key_f7 far\n");
			} else if (aw9620x->sar_num == 1) {
				input_report_key(aw9620x->channels_arr[i].input, KEY_F8, 0);
				AWLOGI(aw9620x->dev, "skey1 report key_f8 far\n");
			}
			break;
		case TRIGGER_TH0:
			...
			if (aw9620x->sar_num == 0) {
				...
				AWLOGI(aw9620x->dev, "skey0 report key_f7 near\n");
			} else if (aw9620x->sar_num == 1) {
				input_report_key(aw9620x->channels_arr[i].input, KEY_F8, 1);
				AWLOGI(aw9620x->dev, "skey1 report key_f8 near\n");
			}
			break;
		case TRIGGER_TH1:
...
static int32_t aw9620x_irq_init(struct aw9620x *aw9620x)
{
	...
	snprintf(irq_gpio_name, sizeof(irq_gpio_name),
					"aw9620x_sar%d_gpio", aw9620x->sar_num);
		...
		if (ret) {
			...
			if (aw9620x->sar_num == 0) {
				ret = devm_request_threaded_irq(aw9620x->dev,
							...
							"aw9620x_sar0_gpio", aw9620x);
			} else if (aw9620x->sar_num == 1) {
				ret = devm_request_threaded_irq(aw9620x->dev,
							aw9620x->to_irq, NULL,
							aw9620x_irq, irq_flags,
							"aw9620x_sar1_gpio", aw9620x);
			} else {
				ret = devm_request_threaded_irq(aw9620x->dev,
							aw9620x->to_irq, NULL,
							aw9620x_irq, irq_flags,
							"aw9620x_sar_gpio", aw9620x);
			}
			if (ret != 0) {
				AWLOGE(aw9620x->dev,
...
static void aw9620x_update_work(struct work_struct *work)
{
	...
	if ((aw9620x->init_err_flag == AW_ERR_CHIPID ||
		...
		AWLOGD(aw9620x->dev, "init_err_flag %d", aw9620x->init_err_flag);
...
static uint32_t reg_data_1cfc_val = 0x1c20;
static ssize_t aw9620x_aot_cali_set(struct device *dev,
						struct device_attribute *attr,
						const char *buf, size_t count)
{
	...
	ret = aw9620x_i2c_read(aw9620x, 0x1CFC, &reg_data_1cfc_val);
	if (ret < 0) {
		AWLOGE(aw9620x->dev, "read 0x1CFC reg failed %d", ret);
	}

	AWLOGE(aw9620x->dev, "read 0x1CFC reg 0x%x", reg_data_1cfc_val);
	ret = aw9620x_i2c_write(aw9620x, 0x1CFC, 0x00);  // disable auto cali
	if (ret < 0) {
		AWLOGE(aw9620x->dev, "set 0x1CFC 0 failed %d", ret);
	}
				...
				struct device_attribute *attr, char *buf)
{
	...
	ret = aw9620x_i2c_write(aw9620x, 0x1CFC, reg_data_1cfc_val);  // enable auto cali again
	if (ret < 0) {
		AWLOGE(aw9620x->dev, "set 0x1CFC to %d failed %d", reg_data_1cfc_val, ret);
	}
...
static DEVICE_ATTR(aot_cali, 0664, NULL, aw9620x_aot_cali_set);
...
static int aw9620x_old_boot_nvr_modify(struct aw9620x *aw9620x)
{
	...
	if (ret != AW_OK) {
		...
		AWLOGD(aw9620x->dev, "i2c new addr0: 0x%x", aw9620x->i2c->addr);
		ret = aw9620x_i2c_write(aw9620x, REG_MCFG00, REG_MCFG00_CPU_HALT);
		if (ret != AW_OK) {
			AWLOGD(aw9620x->dev, "REG_MCFG00 write err %d!", ret);
			return -AW_ERR;
		}
	}
	...
	AWLOGD(aw9620x->dev, "REG[BOOT_LOADER_ACTIVE] = 0x%08x", reg_val);
	if (reg_val != 0) {
...
static int aw9620x_i2c_probe(struct i2c_client *i2c, const struct i2c_device_id *id)
{
	...
	//3. led class register
	if (aw9620x->sar_num == 0){
		aw9620x->cdev.name = "skey0";
	} else {
		aw9620x->cdev.name = "skey1";
	}
...
#ifdef CONFIG_VENDOR_ZTE_MISC_COMMON
	if (aw9620x->sar_num == 0)  /* only add boardtest for main skey */
		zte_misc_register_callback(&is_skey_exist_node, NULL);
#endif
...
err_sysfs:
	devm_led_classdev_unregister(aw9620x->dev, &aw9620x->cdev);
err_class:
#ifdef AW_POWER_ON
err_get_voltage:
...
MODULE_DESCRIPTION("aw9620x SKEY/SAR Driver");
