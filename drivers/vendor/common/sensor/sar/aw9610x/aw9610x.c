...
static uint8_t is_sar1_exist = 0;
...
static ssize_t delay_show(struct class *class,
		...
		char *buf)
{
	pr_info("aw9610x class sar %s: enter\n", __func__);
		...
		const char *buf, size_t count)
{
	...
	if (!aw_sar_ptr) {
		pr_err("aw9610x class %s aw_sar_ptr null!\n", __func__);
		return count;
	}

	if (sscanf(buf, "%d", &en) != 1) {
		pr_err("aw9610x class %s - The number of data are wrong\n", __func__);
		return -EINVAL;
	}
	pr_info("aw9610x class %s en = %d", __func__, en);
...
static ssize_t chip_info_show(struct class *class,
		...
		char *buf)
{
	pr_info("aw9610x class sar %s: enter, chip=%s\n", __func__, chip_info);
...
static ssize_t batch_show(struct class *class,
		...
		char *buf)
{
	pr_info("aw9610x class %s entry", __func__);
		...
		char *buf)
{
	...
	if (aw_sar_ptr) {
		...
		pr_info("aw9610x class %s calibrate reg value is 0x%x\n", __func__, reg_data);
		return snprintf(buf, 64, "%x\n", reg_data);
	}
		...
		const char *buf, size_t count)
{
	...
	if (aw_sar_ptr) {
		aw9610x_i2c_read(aw_sar_ptr, REG_SCANCTRL0, &reg_data);
		pr_info("aw9610x class %s calibrate reg before value is 0x%x\n", __func__, reg_data);
		reg_data |= 0x0F00;
		pr_info("aw9610x class %s calibrate reg after value is 0x%x\n", __func__, reg_data);
