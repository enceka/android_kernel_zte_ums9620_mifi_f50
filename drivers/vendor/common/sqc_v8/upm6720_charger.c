...
do {											\
    if (upm->mode == UPM6720_ROLE_MASTER)						\
        printk(KERN_ERR "[ZTE_LDD_CP][UPM6720-MASTER]:%s:" fmt, __func__, ##__VA_ARGS__);	\
    else if (upm->mode == UPM6720_ROLE_SLAVE)					\
        printk(KERN_ERR "[ZTE_LDD_CP][UPM6720-SLAVE]:%s:" fmt, __func__, ##__VA_ARGS__);	\
    else										\
        printk(KERN_ERR "[ZTE_LDD_CP][UPM6720-STANDALONE]:%s:" fmt, __func__, ##__VA_ARGS__);\
} while(0);

#define upm_info(fmt, ...)								\
do {											\
    if (upm->mode == UPM6720_ROLE_MASTER)						\
        printk(KERN_INFO "[ZTE_LDD_CP][UPM6720-MASTER]:%s:" fmt, __func__, ##__VA_ARGS__);	\
    else if (upm->mode == UPM6720_ROLE_SLAVE)					\
        printk(KERN_INFO "[ZTE_LDD_CP][UPM6720-SLAVE]:%s:" fmt, __func__, ##__VA_ARGS__);	\
    else										\
        printk(KERN_INFO "[ZTE_LDD_CP][UPM6720-STANDALONE]:%s:" fmt, __func__, ##__VA_ARGS__);\
} while(0);

#define upm_dbg(fmt, ...)								\
do {											\
    if (upm->mode == UPM6720_ROLE_MASTER)						\
        printk(KERN_DEBUG "[ZTE_LDD_CP][UPM6720-MASTER]:%s:" fmt, __func__, ##__VA_ARGS__);	\
    else if (upm->mode == UPM6720_ROLE_SLAVE)					\
        printk(KERN_DEBUG "[ZTE_LDD_CP][UPM6720-SLAVE]:%s:" fmt, __func__, ##__VA_ARGS__);	\
    else										\
        printk(KERN_DEBUG "[ZTE_LDD_CP][UPM6720-STANDALONE]:%s:" fmt, __func__, ##__VA_ARGS__);\
} while(0);
...
__maybe_unused static int upm6720_write_byte(struct upm6720 *upm, u8 reg, u8 data)
{
	int ret;
...
static int upm6720_enable_charge(struct upm6720 *upm, bool enable)
{
	...
	upm_info("upm6720 charger %s\n", enable == false ? "disable" : "enable");
	return upm6720_update_bits(upm, UPM6720_REG_0F,
							   UPM6720_CHG_EN_MASK, val);
}

__maybe_unused static int upm6720_check_charge_enabled(struct upm6720 *upm, bool *enable)
{
	int ret = 0;
	u8 val = 0, val1 = 0;
...
static int upm6720_get_charge_enabled(void *arg, unsigned int *en)
{
	int ret = 0;
	u8 val = 0, val1 = 0;

	struct upm6720 *upm = (struct upm6720 *)arg;
	...
	upm_info("%s Reg[%d]=%d Reg[%d]=%d ret=%d\n",
		__func__, UPM6720_REG_0F, val, UPM6720_REG_17, val1, ret);

	return ret;
}

static int upm6720_set_charge_enable(void *arg, unsigned int en)
{
	int ret = 0;
	bool enable = false;
	unsigned int en_now = 0;
	struct upm6720 *upm = (struct upm6720 *)arg;

	enable = (en != 0);

	upm->chip_is_enable = enable;

	/*check is already enable?*/
    ret = upm6720_get_charge_enabled(upm, &en_now);
	if (ret) {
		upm_err("sqc_upm6720_get_enable_chg error\n");
		return ret;
	}

	if ( enable == !!en_now) {
		upm->chip_enabled = !!en;
		upm_info("charger is already en:%d\n", en);
		return 0;
	}

	ret = upm6720_enable_charge(upm, enable);
	if (ret) {
...
static int upm6720_set_batovp(void *arg, unsigned int mV)
{
	struct upm6720 *upm = (struct upm6720 *)arg;
	upm6720_set_batovp_th(upm, mV);

	return 0;
}

static int upm6720_get_batovp_th(void *arg, unsigned int *mV)
{
	u8 value = 0;
	int ret = 0;
	...
	ret = upm6720_read_byte(upm, UPM6720_REG_00, &value);
	if (ret) {
		upm_err("get_batovp error ret=%d\n", ret);
		return -1;
	}

	value = (value & UPM6720_BAT_OVP_MASK) >> UPM6720_BAT_OVP_SHIFT;

	*mV = (value * UPM6720_BAT_OVP_LSB + UPM6720_BAT_OVP_BASE + UPM6720_BAT_OVP_OFFSET) / 1000;

	upm_info("reg(0x%02X)=0x%02X, batovp=%d(mV)\n", UPM6720_REG_00, value, *mV);
...
static int upm6720_get_chg_type(void *arg, unsigned int *chg_type)
{
	arg = arg ? arg : NULL;
...
static int upm6720_set_batocp(void *arg, unsigned int mA)
{
	struct upm6720 *upm = (struct upm6720 *)arg;
	upm6720_set_batocp_th(upm, mA);

	return 0;
}

static int upm6720_get_batocp_th(void *arg, unsigned int *mA)
{
	u8 value = 0;
	int ret = 0;

	struct upm6720 *upm = (struct upm6720 *)arg;
	ret = upm6720_read_byte(upm, UPM6720_REG_02, &value);
	if (ret) {
		upm_err("get_batocp error ret=%d\n", ret);
		return -1;
	}

	value = (value & UPM6720_BAT_OCP_MASK ) >> UPM6720_BAT_OCP_SHIFT;

	*mA = value * UPM6720_BAT_OCP_LSB + UPM6720_BAT_OCP_OFFSET + UPM6720_BAT_OCP_BASE;

	upm_info("reg(0x%02X)=0x%02X, batocp=%d(mA)\n", UPM6720_REG_02, value, *mA);
...
static int upm6720_set_busovp(void *arg, unsigned int mV)
{
	struct upm6720 *upm = (struct upm6720 *)arg;
	upm6720_set_busovp_th(upm, CHG_SW_CAP_MODE, mV);

	return 0;

}

/*defult use CHG_SW_CAP_MODE*/
static int upm6720_get_busovp_th(void *arg, unsigned int *mV)
{
	u8 value = 0;
	int ret = 0;
	...
	ret = upm6720_read_byte(upm, UPM6720_REG_06, &value);
	if (ret) {
		upm_err("get_busovp error ret=%d\n", ret);
		return -1;
	}

	value = (value & UPM6720_BUS_OVP_MASK ) >> UPM6720_BUS_OVP_SHIFT;

	*mV = value * UPM6720_SW_CAP_BUS_OVP_LSB + UPM6720_SW_CAP_BUS_OVP_OFFSET + UPM6720_SW_CAP_BUS_OVP_BASE;

	upm_info("reg(0x%02X)=0x%02X, busovp=%d(mV)\n", UPM6720_REG_06, value, *mV);
...
static int upm6720_set_busocp(void *arg, unsigned int mA)
{
	struct upm6720 *upm = (struct upm6720 *)arg;
	int ret = 0;

	ret = upm6720_set_busocp_th(upm, CHG_SW_CAP_MODE, mA);
	if (ret) {
		upm_err("%s error ret=%d", __func__, ret);
		return -1;
	}

	upm_info("%s busocp=%d(mA)\n", __func__, mA);

	return 0;
}

static int upm6720_get_busocp_th(void *arg, unsigned int *mA)
{
	u8 value = 0;
	int ret = 0;
	...
	ret = upm6720_read_byte(upm, UPM6720_REG_08, &value);
	if (ret) {
		upm_err("get_busocp error ret=%d\n", ret);
		return -1;
	}

	value = (value & UPM6720_BUS_OCP_MASK ) >> UPM6720_BUS_OCP_SHIFT;

	*mA = value * UPM6720_SW_CAP_BUS_OCP_LSB + UPM6720_SW_CAP_BUS_OCP_OFFSET + UPM6720_SW_CAP_BUS_OCP_BASE;

	upm_info("reg(0x%02X)=0x%02X, busocp=%d(mA)\n", UPM6720_REG_08, value, *mA);
...
__maybe_unused static int upm6720_set_vac1ovp(void *arg, int mV)
{
	struct upm6720 *upm = (struct upm6720 *)arg;
	upm6720_set_vac1ovp_th(upm, mV);

	return 0;
}

__maybe_unused static int upm6720_get_vac1ovp_th(void *arg, int *mV)
{
	u8 val = 0;
	int ret = 0;

	struct upm6720 *upm = (struct upm6720 *)arg;
	ret = upm6720_read_byte(upm, UPM6720_REG_0E, &val);
	if (ret) {
		upm_err("get_batovp error ret=%d\n", ret);
		return -1;
	}

	if (val == UPM6720_AC1_OVP_18V) {
		*mV = 18000;
	} else if (val == UPM6720_AC1_OVP_16V) {
		*mV = 16000;
	} else if (val == UPM6720_AC1_OVP_14V) {
		*mV = 14000;
	} else if (val == UPM6720_AC1_OVP_12V) {
		*mV = 12000;
	} else if (val == UPM6720_AC1_OVP_10V5) {
		*mV = 10500;
	} else if (val == UPM6720_AC1_OVP_6V5) {
		*mV = 6500;
	} else {
		upm_err("vac1ovp val err is%d", val);
		return -1;
	}
	upm_info("upm6720_get_vac1ovp_th, vac1ovp_th is %d mV", *mV);
	return 0;
}
...
static int upm6720_set_bat_ovp_alm(void *arg, unsigned int mV)
{
	struct upm6720 *upm = (struct upm6720 *)arg;

	upm6720_set_bat_ovp_alm_th(upm, mV);

	return 0;

}

static int upm6720_get_bat_ovp_alm_th(void *arg, unsigned int *mV)
{
	u8 value = 0;
	int ret = 0;
	...
	ret = upm6720_read_byte(upm, UPM6720_REG_01, &value);
	if (ret) {
		upm_err("get_bat_ovp_alm error ret=%d\n", ret);
		return -1;
	}

	value = (value & UPM6720_BAT_OVP_MASK) >> UPM6720_BAT_OVP_SHIFT;

	*mV = value * UPM6720_BAT_OVP_ALM_LSB + UPM6720_BAT_OVP_ALM_BASE + UPM6720_BAT_OVP_ALM_OFFSET;

	upm_info("reg(0x%02X)=0x%02X, batovp=%d(mV)\n", UPM6720_REG_01, value, *mV);
...
static int upm6720_set_bat_ocp_alm(void *arg, unsigned int mA)
{
	struct upm6720 *upm = (struct upm6720 *)arg;
	upm6720_set_bat_ocp_alm_th(upm, mA);

	return 0;
}

static int upm6720_get_bat_ocp_alm_th(void *arg, unsigned int *mA)
{
	u8 value = 0;
	int ret = 0;

	struct upm6720 *upm = (struct upm6720 *)arg;
	ret = upm6720_read_byte(upm, UPM6720_REG_03, &value);
	if (ret) {
		upm_err("get_bat_ocp_alm error ret=%d\n", ret);
		return -1;
	}

	value = (value & UPM6720_BAT_OCP_ALM_MASK) >> UPM6720_BAT_OCP_ALM_SHIFT;

	*mA = value * UPM6720_BAT_OCP_ALM_LSB + UPM6720_BAT_OCP_ALM_BASE + UPM6720_BAT_OCP_ALM_OFFSET;

	upm_info("reg(0x%02X)=0x%02X, batocp=%d(mA)\n", UPM6720_REG_03, value, *mA);
...
static int upm6720_set_bus_ovp_alm_th(struct upm6720 *upm, chg_mode_t mode, int threshold)
{
	...
	if (mode == CHG_SW_CAP_MODE) {
		...
		val = (threshold - UPM6720_SW_CAP_BUS_OVP_ALM_BASE - UPM6720_SW_CAP_BUS_OVP_ALM_OFFSET) /
			  UPM6720_SW_CAP_BUS_OVP_ALM_LSB;
	} else {
		...
		val = (threshold - UPM6720_BYPASS_BUS_OVP_ALM_BASE - UPM6720_BYPASS_BUS_OVP_ALM_OFFSET) /
			  UPM6720_BYPASS_BUS_OVP_ALM_LSB;
	}
...
static int upm6720_set_bus_ovp_alm(void *arg, unsigned int mA)
{
	struct upm6720 *upm = (struct upm6720 *)arg;
	upm6720_set_bus_ovp_alm_th(upm, CHG_SW_CAP_MODE, mA);

	return 0;
}

static int upm6720_get_bus_ovp_alm_th(void *arg, unsigned int *mV)
{
	u8 value = 0;
	int ret = 0;

	struct upm6720 *upm = (struct upm6720 *)arg;
	ret = upm6720_read_byte(upm, UPM6720_REG_07, &value);
	if (ret) {
		upm_err("bus_ovp_alm error ret=%d\n", ret);
		return -1;
	}

	value = (value & UPM6720_BUS_OVP_ALM_MASK ) >> UPM6720_BUS_OVP_ALM_SHIFT;
	*mV = value * UPM6720_BYPASS_BUS_OVP_ALM_LSB + UPM6720_SW_CAP_BUS_OVP_ALM_OFFSET + UPM6720_BYPASS_BUS_OVP_ALM_BASE;

	upm_info("reg(0x%02X)=0x%02X, bus_ovp_alm=%d(mV)\n", UPM6720_REG_07, value, *mV);
...
static int upm6720_set_bus_ocp_alm(void *arg, unsigned int mA)
{
	struct upm6720 *upm = (struct upm6720 *)arg;
	upm6720_set_bus_ocp_alm_th(upm, mA);

	return 0;

}

static int upm6720_get_bus_ocp_alm_th(void *arg, unsigned int *mA)
{
	u8 value = 0;
	int ret = 0;
	...
	ret = upm6720_read_byte(upm, UPM6720_REG_09, &value);
	if (ret) {
		upm_err("bus_ocp_alm error ret=%d\n", ret);
		return -1;
	}

	value = (value & UPM6720_BUS_OCP_ALM_MASK ) >> UPM6720_BUS_OCP_ALM_SHIFT;
	*mA = value * UPM6720_BUS_OCP_ALM_LSB + UPM6720_BUS_OCP_ALM_OFFSET + UPM6720_BUS_OCP_ALM_BASE;

	upm_info("reg(0x%02X)=0x%02X, bus_ocp_alm=%d(mV)\n", UPM6720_REG_09, value, *mA);
...
static int upm6720_get_adc_data(struct upm6720 *upm, adc_channel_t channel,  int *result)
{
	...
	case UPM_ADC_IBUS:	// !!!only switched cap mode, bypass mode will do later
		sval = val * UPM6720_IBUS_ADC_SW_CAP_LSB / UPM6720_IBUS_ADC_SW_CAP_PRECISION + UPM6720_IBUS_ADC_SW_CAP_BASE +
			   UPM6720_IBUS_ADC_SW_CAP_OFFSET;
		break;
	case UPM_ADC_VBUS:
...
static int upm6720_init_device(struct upm6720 *upm)
{
	...
	upm6720_update_bits(upm, UPM6720_REG_0F,
						UPM6720_DIS_ACDRV_BOTH_MASK, 0 << UPM6720_DIS_ACDRV_BOTH_SHIFT);
	upm6720_read_byte(upm, UPM6720_REG_0F, &val);
	upm_info("init UPM6720_REG_0F=0x%x\n", val);

	return 0;
}

static int upm6720_init_reg(void *arg)
{
	struct upm6720 *upm = (struct upm6720 *)arg;
...
__maybe_unused static int upm6720_set_present(struct upm6720 *upm, bool present)
{
...
static int upm6720_get_ibat_ma(void *arg, unsigned int *mA)
{
	int ret = 0;
	int result = 0;
	struct upm6720 *upm = (struct upm6720 *)arg;
	...
	if (!ret) {
		*mA = (unsigned int)result;
		upm->ibat_curr = *mA;
		upm_info("upm6720_get_ibat_ma is %d ma", *mA);
	}
	return 0;
}

static int upm6720_get_vbat_mv(void *arg, unsigned int *mV)
{
	int ret = 0;
	int result = 0;
	struct upm6720 *upm = (struct upm6720 *)arg;
	...
	if (!ret) {
		*mV = (unsigned int)result;
		upm->vbat_volt = *mV;
		upm_info("upm6720_get_vbat_mv is %d mV", *mV);
	}
	return 0;
}

static int upm6720_get_ibus_ma(void *arg, unsigned int *mA)
{
	int ret = 0;
	int result = 0;
	struct upm6720 *upm = (struct upm6720 *)arg;
	...
	if (!ret) {
		*mA = (unsigned int)result;
		upm->ibus_curr = *mA;
		upm_info("upm6720_get_ibus_ma is %d ma", *mA);
	}
	return 0;
}

static int upm6720_get_vbus_mv(void *arg, unsigned int *mV)
{
	int ret = 0;
	int result = 0;
	struct upm6720 *upm = (struct upm6720 *)arg;
	...
	if (!ret) {
		*mV = (unsigned int)result;
		upm->vbus_volt = *mV;
		upm_info("upm6720_get_vbus_mv is %d mV", *mV);
	}
	return 0;
}
...
static void upm6720_interrupt_work(struct work_struct *work)
{
	struct upm6720 *upm  = NULL;

	upm = container_of(work, struct upm6720, irq_work);
	pm_stay_awake(upm->dev);
...
static irqreturn_t upm6720_charger_interrupt(int irq, void *_chip)
{
	struct upm6720 *upm = _chip;

	if (upm == NULL) {
		upm_err("chip is null\n");
		return -1;
	}

	if (upm->chip_already_init == 0)
		upm_err("upm chip not init\n");

	disable_irq_nosync(upm->irq);
	schedule_work(&upm->irq_work);

	return IRQ_HANDLED;
}
#endif

static int upm6720_get_irq_gpio(struct upm6720 *upm)
{
	int ret;
...
static int upm6720_get_chg_status(void *arg, unsigned int *abnormal_stat)
{
	int ret = 0;
	u8 val = 0;
	struct upm6720 *upm = (struct upm6720 *)arg;
	...
	u8 ac_ovp_val = 0;
	u8 ac_ovp_state = 0;
	u8 vbus_ovp_state_val = 0;
	u8 vbus_ovp_state = 0;
	u8 vbus_ovp_flag_val = 0;
	u8 vbus_ovp_flag = 0;
	int chg_status = 0;
	...
	if (ret) {
		*abnormal_stat = 1;
		upm_err("upm6720_get_chg_status error ret=%d\n", ret);
		return -1;
	}

	ret = upm6720_read_byte(upm, UPM6720_REG_15, &ac_ovp_val);
	if (ret) {
		*abnormal_stat = 1;
		upm_err("get ac_ovp error ret=%d\n", ret);
		return -1;
	}

	ret = upm6720_read_byte(upm, UPM6720_REG_13, &vbus_ovp_state_val);
	if (ret) {
		*abnormal_stat = 1;
		upm_err("get vbus_ovp_state error ret=%d\n", ret);
		return -1;
	}

	ret = upm6720_read_byte(upm, UPM6720_REG_18, &vbus_ovp_flag_val);
	if (ret) {
		*abnormal_stat = 1;
		upm_err("get vbus_ovp_flag error ret=%d\n", ret);
		return -1;
	}

	ac_ovp_state = (ac_ovp_val & UPM6720_VAC1_OVP_STAT_MASK) >> UPM6720_VAC1_OVP_STAT_SHIFT;
	vbus_ovp_state = (vbus_ovp_state_val & UPM6720_BUS_OVP_STAT_MASK) >> UPM6720_BUS_OVP_STAT_SHIFT;
	vbus_ovp_flag = (vbus_ovp_flag_val & UPM6720_BUS_OVP_FLAG_MASK) >> UPM6720_BUS_OVP_FLAG_SHIFT;
	upm_err("ac_ovp_state %d, vbus_ovp_state %d, vbus_ovp_flag %d\n",
			ac_ovp_state,
			vbus_ovp_state,
			vbus_ovp_flag);

	if (ac_ovp_state || vbus_ovp_state || vbus_ovp_flag) {
		chg_status |= BIT(SQC_ERR_VBUS_OVP);
	} else {
		chg_status &= ~BIT(SQC_ERR_VBUS_OVP);
	}

	converter_state = (val & UPM6720_CONV_ACTIVE_STAT_MASK) >> UPM6720_CONV_ACTIVE_STAT_SHIFT;
	if (upm->chip_is_enable) {
		if (converter_state) {
	...
	*abnormal_stat = chg_status;
	upm_err("switching abnormal_stat %d\n", *abnormal_stat);
	return 0;
}

static int upm6720_get_interrupt_status(void *arg, unsigned int *status)
{
	struct upm6720 *upm = (struct upm6720 *)arg;
	int ret = 0;
...
static struct sqc_pmic_chg_ops sqc_upm6720_chg_ops = {

	.init_pmic_charger = upm6720_init_reg,
	.get_chg_type = upm6720_get_chg_type,
	.chg_enable = upm6720_set_charge_enable,
	.chg_enable_get = upm6720_get_charge_enabled,

	.get_chg_status = upm6720_get_chg_status,
	.get_int_status = upm6720_get_interrupt_status,

	.chg_role_set = NULL,
	.chg_role_get = NULL,

	/*battery*/
	.batt_ovp_volt_set = upm6720_set_batovp,
	.batt_ovp_volt_get = upm6720_get_batovp_th,
	.batt_ovp_alm_volt_set = upm6720_set_bat_ovp_alm,
	.batt_ovp_alm_volt_get = upm6720_get_bat_ovp_alm_th,
	.batt_ocp_curr_set = upm6720_set_batocp,
	.batt_ocp_curr_get = upm6720_get_batocp_th,
	.batt_ocp_alm_curr_set = upm6720_set_bat_ocp_alm,
	.batt_ocp_alm_curr_get = upm6720_get_bat_ocp_alm_th,
	.batt_ibat_get = upm6720_get_ibat_ma,
	.batt_vbat_get = upm6720_get_vbat_mv,
	...
	/*usb bus*/
	.usb_ovp_volt_set = upm6720_set_busovp,
	.usb_ovp_volt_get = upm6720_get_busovp_th,
	.usb_ovp_alm_volt_set = upm6720_set_bus_ovp_alm,
	.usb_ovp_alm_volt_get = upm6720_get_bus_ovp_alm_th,
	.usb_ocp_curr_set = upm6720_set_busocp,
	.usb_ocp_curr_get = upm6720_get_busocp_th,
	.usb_ocp_alm_curr_set = upm6720_set_bus_ocp_alm,
	.usb_ocp_alm_curr_get = upm6720_get_bus_ocp_alm_th,
	.usb_ibus_get = upm6720_get_ibus_ma,
	.usb_vbus_get = upm6720_get_vbus_mv,
};

static int upm6720_slave_chg_id_get(char *val, const void *arg)
{
	struct upm6720 *upm = (struct upm6720 *)arg;
	int slave_chg_id = 0;
	...
	upm_info("cp1_init_finished: %d, cp1_init_finished: %d, slave_chg_id: %d\n",
...
static struct zte_misc_ops slave_chg_id_node = {
	...
	.set = NULL,
	.get = upm6720_slave_chg_id_get,
	.free = NULL,
	.arg = NULL,
};

static int upm6720_chip_name_show(struct seq_file *m, void *v)
{
	struct upm6720 *upm = m->private;
...
static int upm6720_chip_name_open(struct inode *inode, struct file *file)
{
	return single_open(file, upm6720_chip_name_show, PDE_DATA(inode));
}

static const struct file_operations upm6720_chip_name_node = {
	.owner = THIS_MODULE,
	.open = upm6720_chip_name_open,
	.read = seq_read,
	.llseek = seq_lseek,
};

static int upm6720_otg_notifier(struct notifier_block *nb,
				   unsigned long event, void *data)
{
	struct upm6720 *upm =
		container_of(nb, struct upm6720, edv_nb);

	upm_info("extcon otg state: %d\n", extcon_get_state(upm->edev, EXTCON_USB_HOST));
	cancel_delayed_work(&upm->otg_work);
	schedule_delayed_work(&upm->otg_work, msecs_to_jiffies(0));

	return NOTIFY_DONE;
}

static void upm6720_otg_workfunc(struct work_struct *work)
{
	struct delayed_work *dwork = to_delayed_work(work);
	struct upm6720 *upm = container_of(dwork,
							  struct upm6720,
							  otg_work);
	int ret = -1;
	u8 val = 0;
	...
	if (!upm) {
		upm_err("upm is null failed\n");
		return;
	}

	otg_present = extcon_get_state(upm->edev, EXTCON_USB_HOST);
	upm_info("extcon otg_present: %d\n", otg_present);

	ret = upm6720_read_byte(upm, UPM6720_REG_0F, &val);
	if (ret) {
		upm_err("get UPM6720_REG_0F error ret=%d\n", ret);
		return;
	}
	upm_info("UPM6720_REG_0F=0x%x +++++\n", val);

	if (otg_present) {
		upm6720_update_bits(upm, UPM6720_REG_0F,
							UPM6720_EN_OTG_MASK, UPM6720_EN_OTG_ENABLE << UPM6720_EN_OTG_SHIFT);
		upm6720_update_bits(upm, UPM6720_REG_0F,
							UPM6720_DIS_ACDRV_BOTH_MASK, 0 << UPM6720_DIS_ACDRV_BOTH_SHIFT);
		upm6720_update_bits(upm, UPM6720_REG_0F,
							UPM6720_ACDRV1_STAT_MASK, UPM6720_ACDRV1_STAT_ON << UPM6720_ACDRV1_STAT_SHIFT);
	} else {
		upm6720_update_bits(upm, UPM6720_REG_0F,
							UPM6720_EN_OTG_MASK, UPM6720_EN_OTG_DISABLE << UPM6720_EN_OTG_SHIFT);
	}
								 ...
								 const struct i2c_device_id *id)
{
	...
	pr_info("upm6720 probe begin\n");

	if (client == NULL) {
		pr_err("upm6720 client is null\n");
		return -ENOMEM;
	}
	...
	upm->edev = extcon_get_edev_by_phandle(&client->dev, 0);
	if (IS_ERR(upm->edev)) {
	...
	if (ret) {
		upm_err("No upm6720 device found!, ret=%d\n", ret);
		goto err_1;
	}
	...
	ret = upm6720_get_work_mode(upm, &upm->mode);
	if (ret) {
		upm_err("upm6720 get work mode failed, ret=%d\n", ret);
		goto err_1;
	}
	...
	INIT_WORK(&upm->irq_work, upm6720_interrupt_work);
	INIT_DELAYED_WORK(&upm->monitor_work, upm6720_monitor_work);
	//schedule_delayed_work(&upm->monitor_work, msecs_to_jiffies(3*1000));
	if (upm->mode == UPM6720_ROLE_STANDALONE || upm->mode == UPM6720_ROLE_MASTER) {
		INIT_DELAYED_WORK(&upm->otg_work, upm6720_otg_workfunc);
	}

	ret = upm6720_parse_dt(upm, &client->dev);
	if (ret) {
		upm_err("Failed to upm6720 parse dt\n");
		goto err_1;
	}
	...
	ret = upm6720_get_irq_gpio(upm);
	if (ret) {
		upm_err("Failed to upm6720_get_irq_gpio\n");
		goto err_1;
	}

	ret = upm6720_irq_register(upm);
	if (ret) {
		upm_err("Failed to upm6720_irq_register\n");
		goto err_2;
	}
	...
	if (upm->mode == UPM6720_ROLE_SLAVE) {
		...
		memcpy(sqc_ops, &sqc_upm6720_chg_ops, sizeof(struct sqc_pmic_chg_ops));
		sqc_ops->arg = (void *)upm;
		ret = sqc_hal_charger_register(sqc_ops, SQC_CHARGER_PARALLEL2);
		if (ret < 0) {
			upm_err("%s register sqc hal fail(%d)\n", __func__, ret);
			goto err_3;
		}
	} else {
		sqc_ops = kzalloc(sizeof(struct sqc_pmic_chg_ops), GFP_KERNEL);
	...
	if (upm->mode == UPM6720_ROLE_STANDALONE) {
		cp1_init_finished = 1;
		cp2_init_finished = 1;
		zte_misc_register_callback(&slave_chg_id_node, upm);
		upm->name = "upm6720";
		proc_create_data("driver/slave_chg_name", 0664, NULL,
			&upm6720_chip_name_node, upm);
	} else if (upm->mode == UPM6720_ROLE_MASTER) {
		cp1_init_finished = 1;
		zte_misc_register_callback(&slave_chg_id_node, upm);
		upm->name = "upm6720";
	...
	upm->chip_already_init = 1;

	if (upm->mode == UPM6720_ROLE_STANDALONE || upm->mode == UPM6720_ROLE_MASTER) {
		upm->edv_nb.notifier_call = upm6720_otg_notifier;
		ret = extcon_register_notifier(upm->edev, EXTCON_USB_HOST,
							&upm->edv_nb);
		if (ret) {
			upm_err("failed to register extcon USB HOST notifier.\n");
			ret = -ENODEV;
			goto err_3;
		}
	}
...
err_3:
	free_irq(upm->irq, upm);
err_2:
	gpio_free(upm->irq_gpio);
err_1:
